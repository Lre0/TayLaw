    ...
    try {
      ...
      if (response.ok) {
        ...
        if (isComplete) {
          ...
          const resultsResponse = await fetch(`${API_BASE_URL}/api/batch-results/${batchId}`, {
            headers: { 'ngrok-skip-browser-warning': 'true' }
          })
          if (resultsResponse.ok) {
            const resultsText = await resultsResponse.text()
            console.log('Raw results response:', resultsText)
            let results
            try {
              results = JSON.parse(resultsText)
            } catch (parseError) {
              console.error('JSON parse error in results:', parseError)
              console.error('Results response text:', resultsText)
              return
            }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          throw new Error(`Failed to analyze documents: ${response.statusText}. Response: ${errorText}`)
        }
      }
    ...
    <div className="max-w-[95vw] mx-auto px-4 py-6">
      ...
      {(isProcessing || batchResult) && (
        <div className="mb-6">
          ...
          <AgentMonitor isActive={isProcessing || !!batchResult} />
        </div>
      )}
